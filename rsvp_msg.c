...
void send_resv_message(int sock, struct in_addr dest_ip) {
    ...
    db_node *resv_node = search_node(resv_tree, dest_ip, compare_resv_del);
...
void receive_path_message(int sock, char buffer[], struct sockaddr_in sender_addr) {
    ...
    db_node *path_node = search_node(path_tree, session_obj->dst_ip, compare_path_del);
    if(path_node == NULL){
        temp = path_tree_insert(path_tree, buffer);
 	if(temp != NULL) {
		path_tree = temp;
	        path_node = search_node(path_tree, session_obj->dst_ip, compare_path_del);
	}
    }
    ...
    if(path_node != NULL) {
    	...
    	if(strcmp(inet_ntoa(p->nexthop_ip), "0.0.0.0") == 0) {
	        ...
	        db_node *resv_node = search_node(resv_tree, receiver_ip, compare_resv_del);
       		if(resv_node == NULL){
        	...
        	send_resv_message(sock, session_obj->dst_ip);
    	} else {
        	printf("send path msg to nexthop \n");
        	send_path_message(sock, session_obj->dst_ip);
	}
    }
}
...
void send_path_message(int sock, struct in_addr dest_ip) {
    struct sockaddr_in dest_addr;
    ...
    db_node *path_node = search_node(path_tree, dest_ip, compare_path_del);
    path_msg *p = (path_msg*)path_node->data;
...
void receive_resv_message(int sock, char buffer[], struct sockaddr_in sender_addr) {
    ...
    db_node *resv_node = search_node(resv_tree, session_obj->dst_ip, compare_resv_del);
    if(resv_node == NULL){
        temp = resv_tree_insert(resv_tree, buffer, 0);
	if(temp != NULL) {
		resv_tree = temp;
        	resv_node = search_node(resv_tree, session_obj->dst_ip, compare_resv_del);
	}
    }
    display_tree(resv_tree, 0);
    ...
    if(resv_node != NULL) {
        resv_msg *p = (resv_msg*)resv_node->data;
         ...
         db_node* path_node = search_node(path_tree, session_obj->dst_ip, compare_resv_del);
