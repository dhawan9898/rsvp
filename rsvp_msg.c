...
void send_resv_message(int sock, uint16_t tunnel_id, struct in_addr dest_ip) {
    ...
    db_node *resv_node = search_node(resv_tree, tunnel_id, dest_ip, compare_resv_del);
...
void receive_path_message(int sock, char buffer[], struct sockaddr_in sender_addr) {
    ...
    db_node *path_node = search_node(path_tree, session_obj->tunnel_id, session_obj->dst_ip, compare_path_del);
    if(path_node == NULL){
